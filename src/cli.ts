...
program
  ...
  .action(async (uri, options) => {
    try {
      ...
      const imageContent = result.contents.find(item => item.blob && item.mimeType?.includes('image'));
      ...
      if (options.output) {
        ...
        if (options.view) {
          try {
            ...
            console.error('Failed to open image:', error instanceof Error ? error.message : String(error));
          }
        }
      } else {
      ...
      console.error('Error generating resource:', error instanceof Error ? error.message : String(error));
      process.exit(1);
    }
  });

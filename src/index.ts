...
server.tool(
  ...
  'Generate QR codes in various formats with customizable error correction levels and sizes',
  {
    ...
    errorCorrectionLevel: z
      ...
      .optional()
      .default('M'),
    size: z.number().min(1).max(10).describe('Size of the QR code (1-10)').optional().default(3),
    format: z
      .enum(['image', 'text'])
  ...
  async (args, _extra) => {
    try {
      ...
      if (format === 'text') {
        ...
        const imageData = result.data.startsWith('data:') ? result.data.split(',')[1] : result.data
  ...
  return {
    resourceTemplates: [
      ...
      },
      {
        ...
        example:
          'qrcode://calendar?title=Team%20Meeting&start=2025-03-15T10:00:00&end=2025-03-15T11:00:00&location=Conference%20Room&description=Weekly%20team%20sync',
        inputSchema: {
          type: 'object',
          properties: {
            title: {
    ...
    prompts: [
      {
        ...
        prompt:
          "Generate a QR code that allows people to connect to my home WiFi network called 'MyHomeNetwork' with password 'secure123' using WPA encryption",
      },
      {
        name: 'create-contact-qr',
        description: 'Create a QR code with contact information',
        prompt:
          'Create a QR code with my contact information: name: Jane Smith, phone: 555-987-6543, email: jane@example.com, organization: ABC Corp, title: Senior Developer',
      },
      {
        name: 'create-url-qr',
      ...
      },
      {
        ...
        prompt:
          "Generate a QR code with this text message: 'Thank you for visiting our booth at the conference! Use discount code CONF2025 for 20% off your next purchase.'",
      },
      {
        name: 'create-calendar-qr',
        description: 'Create a QR code for a calendar event',
        prompt:
          "Create a QR code for our team meeting on March 15, 2025 from 10:00 AM to 11:30 AM in Conference Room A with the description 'Quarterly planning session'",
      },
      {
        name: 'create-custom-sized-qr',
        description: 'Create a QR code with custom size settings',
        prompt:
          'Generate a large QR code (size 8) for my business URL https://mybusiness.example.com',
      },
      {
        name: 'qr-with-high-error-correction',
        description: 'Create a QR code with high error correction level',
        prompt:
          'Create a QR code for https://example.org with the highest possible error correction level for maximum reliability',
      },
      {
        name: 'text-output-qr',
        description: 'Create a QR code with text output format',
        prompt:
          "Generate a QR code for 'Hello World' and display it as ASCII/text art that I can paste into a document",
      },
    ],
  }
})

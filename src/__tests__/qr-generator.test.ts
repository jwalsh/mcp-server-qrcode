...
describe('QR Code Generator', () => {
  describe('generateQRCode', () => {
    ...
    it('should generate a QR code in utf8 text format', async () => {
      const result = await generateQRCode({ content: 'https://example.com', format: 'utf8' })

      expect(result).toBeTruthy()
      expect(['utf8', 'terminal']).toContain(result.format)
      expect(typeof result.data).toBe('string')
      expect(result.data.length).toBeGreaterThan(0)
    })

    it('should generate a QR code in terminal format', async () => {
      const result = await generateQRCode({ content: 'Hello Terminal', format: 'terminal' })
    ...
    it('should escape single quotes in content without failing', async () => {
      const result = await generateQRCode({ content: "O'Reilly & Sons", format: 'utf8' })
    ...
    it('should throw an error for an unsupported format', async () => {
      await expect(
        // Bypass the type system to exercise the unsupported-format branch
        generateQRCode({ content: 'https://example.com', format: 'svg' as never })

...
describe('QR Code Templates and Prompts', () => {
  ...
  test('server has examples for templates', () => {
    // Check for the example URIs (template examples may be line-wrapped by prettier)
    expect(sourceCode).toContain('qrcode://text?content=')
    expect(sourceCode).toContain('qrcode://calendar?title=')
  })
